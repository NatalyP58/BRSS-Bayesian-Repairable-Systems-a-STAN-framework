...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: ARA application to sugarcane harvester data
# ------------------------------------------------------------
# This script applies the Arithmetic Reduction of Age (ARA)
# model to a selected vehicle from the sugarcane harvester
# dataset.
#
...
calc_AIC_BIC <- function(results, times, time_trunc) {
  ...
  loglik_ind <- log_like_ARA(results$bayes_est_ind, time = times, time_trunc = time_trunc)
  AIC_ind <- 2 * k - 2 * loglik_ind
  BIC_ind <- k * log(n) - 2 * loglik_ind
  
  # Bayesian estimates with dependent priors
  loglik_dep <- log_like_ARA(results$bayes_est_dep, time = times, time_trunc = time_trunc)
