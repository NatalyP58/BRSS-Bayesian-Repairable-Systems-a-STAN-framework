...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: ARI simulation with parallel cluster execution
# ------------------------------------------------------------
# This script runs parallel simulation experiments for the
# Arithmetic Reduction of Intensity (ARI) model, including:
#   1. data generation,
...
run_simulation_parallel_cluster <- function(m, k, param, time_trunc, SEED = NULL) {
  ...
  single_run <- function(i) {
    ...
    list(
      ...
      fit_ind       = if (bayes_ind$success) bayes_ind$fit else NULL,
      ...
      fit_dep       = if (bayes_dep$success) bayes_dep$fit else NULL,
