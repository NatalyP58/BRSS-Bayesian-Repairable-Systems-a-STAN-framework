...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: PIR application to sugarcane harvester data
# ------------------------------------------------------------
# This script applies the Partial Imperfect Repair (PIR) model
# to a selected vehicle from the sugarcane harvester dataset.
#
...
# Rename subsystems for presentation
...
calc_AIC_BIC <- function(results, times, tau, time_trunc) {
  ...
  loglik_ind <- log_like_PIR(results$bayes_est_ind, times, tau, time_trunc)
  AIC_ind <- 2 * k - 2 * loglik_ind
  BIC_ind <- k * log(n) - 2 * loglik_ind
  
  # Bayesian estimates with dependent priors
  loglik_dep <- log_like_PIR(results$bayes_est_dep, times, tau, time_trunc)
...
ic_vals <- calc_AIC_BIC(
  results,
  times = times_offseason,
  tau   = tau,
  time_trunc = time_trunc
)
print(ic_vals)
...
calc_DIC(results$fit_ind, times = times_offseason, tau = tau, time_trunc = time_trunc)
calc_DIC(results$fit_dep, times = times_offseason, tau = tau, time_trunc = time_trunc)
