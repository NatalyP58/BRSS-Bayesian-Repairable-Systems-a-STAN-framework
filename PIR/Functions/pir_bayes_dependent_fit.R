...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: PIR Bayesian estimation with dependent priors
# ------------------------------------------------------------
# This file implements Bayesian estimation for the Partial
# Imperfect Repair (PIR) model using Stan under a
# dependent-prior specification based on an LKJ correlation
# structure.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Bayesian estimation for the PIR model with dependent priors
# ------------------------------------------------------------
bayesian_estimation_dep <- function(time, time_trunc, beta_r = NULL, mu_T_r = NULL,
  ...
  stan_data <- list(
    ...
    N            = length(tau),                               # number of change points
    ...
    tau          = array(tau, dim = length(tau)),             # fixed change points
  ...
  fit <- sampling(
    ...
    warmup = 1000,                                         # iterations discarded for adaptation
