# Simulation Module for the Partial Imperfect Repair (PIR) Model

This folder contains the simulation workflow for the **Partial Imperfect Repair (PIR)** model developed in the manuscript:
...
- `pir_simulation_cluster.R`  
- `pir_simulation_results.R`  
...
- generates failure-time data under the PIR model,  
- fits the PIR model using three estimation methods,  
...
### `run_simulation_parallel_cluster(m, k, param, time_trunc, tau, SEED = NULL)`
...
- `tau`  
  Vector of change points defining the PIR structure.  
