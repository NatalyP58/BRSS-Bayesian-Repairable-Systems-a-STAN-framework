...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: PIR simulation results summary
# ------------------------------------------------------------
# This script summarizes simulation results for the Partial
# Imperfect Repair (PIR) model. It computes bias, mean squared
# error (MSE), and empirical coverage for the frequentist
# estimator and the two Bayesian estimators:
...
sim_df <- function(sim, param) {
  ...
  colnames(simul) <- c(
    "Method",
    "Bias_beta", "Bias_mu_T", "Bias_theta",
    "MSE_beta", "MSE_mu_T", "MSE_theta"
  )
...
sim_df_coverage <- function(sim) {
  ...
  colnames(simul) <- c("Method", "HPD_beta", "HPD_mu_T", "HPD_theta")
  return(simul)
}
