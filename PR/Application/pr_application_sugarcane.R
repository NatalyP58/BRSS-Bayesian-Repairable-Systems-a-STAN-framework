...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: PR application to sugarcane harvester data
# ------------------------------------------------------------
# This script applies the Perfect Repair (PR) model to a
# selected vehicle from the sugarcane harvester dataset.
#
...
source_files <- c(
  file.path("functions", "pr_model_functions.R"),
  file.path("functions", "pr_stan_models.R"),
  file.path("functions", "pr_frequentist_fit.R"),
  file.path("functions", "pr_bayes_dependent_fit.R"),
  file.path("functions", "pr_bayes_independent_fit.R")
)
...
calc_AIC_BIC <- function(results, times, time_trunc) {
  ...
  k <- length(results$bayes_est_ind)
  ...
  loglik_ind <- log_like_PR(results$bayes_est_ind, time = times, time_trunc = time_trunc)
  AIC_ind <- 2 * k - 2 * loglik_ind
  BIC_ind <- k * log(n) - 2 * loglik_ind
  
  # Bayesian estimates with dependent priors
  loglik_dep <- log_like_PR(results$bayes_est_dep, time = times, time_trunc = time_trunc)
