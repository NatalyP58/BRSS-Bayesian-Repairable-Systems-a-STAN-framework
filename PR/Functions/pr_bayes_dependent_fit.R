...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: PR Bayesian fit with dependent priors
# ------------------------------------------------------------
# This file implements Bayesian estimation for the Perfect
# Repair (PR) model using Stan under a dependent-prior
# specification based on an LKJ correlation structure.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Bayesian estimation for the PR model with dependent priors
# ------------------------------------------------------------
bayesian_estimation_dep <- function(time, time_trunc, beta_r = NULL, mu_T_r = NULL,
  ...
  if (is.null(compiled_model)) {
    if (!exists("model.compiled.dep")) {
      stop("model.compiled.dep not found in the global environment")
    }
  ...
  stan_data <- list(
    ...
    beta0        = 1,                                         # baseline value for beta
    mu0          = 20,                                        # baseline value for mu_T
    sigma_beta0  = 2,                                         # upper bound for the prior scale of beta
    sigma_mu0    = 50,                                        # upper bound for the prior scale of mu_T
    eta          = 1                                          # LKJ shape parameter (uniform over correlations)
  )
  
  # Posterior sampling under the dependent-prior specification
  fit <- sampling(
    ...
    control = list(adapt_delta = 0.9, max_treedepth = 15), # HMC tuning parameters
  ...
  # Posterior point estimates based on posterior medians
  estimate <- c(median(beta_est), median(mu_T_est))
  
  if (simulation) {
    # 95% HPD intervals used to evaluate empirical coverage
