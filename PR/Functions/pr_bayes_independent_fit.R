...
# Date: April 15, 2026
# ------------------------------------------------------------
# File: PR Bayesian fit with independent priors
# ------------------------------------------------------------
# This file implements Bayesian estimation for the Perfect
# Repair (PR) model using Stan under an independent-prior
# specification.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Bayesian estimation for the PR model with independent priors
# ------------------------------------------------------------
bayesian_estimation_ind <- function(time, time_trunc, beta_r = NULL, mu_T_r = NULL,
  ...
  stan_data <- list(
    ...
    beta0      = c(0.16, 0.04),                            # Gamma(shape=0.16, rate=0.04) prior for beta_raw
    # E[beta_raw] = 4, Var(beta_raw) = 100
    mu0        = c(2, 0.05)                                # Gamma(shape=2, rate=0.05) prior for mu_T
    # E[mu_T] = 40, Var(mu_T) = 800
  )
